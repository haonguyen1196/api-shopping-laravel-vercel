                ...
                <form action="{{ route('admin.role.store') }}" method="post">
                  @csrf
                    <div class="form-group">
                        ...
                        <input value="{{ old('name') }}" type="text" class="form-control @error('name') is-invalid @enderror" name="name" placeholder="Nhập tên vai trò">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('display_name') is-invalid @enderror" rows="4" name="display_name">{{ old('display_name') }}</textarea>
                        @error('display_name')
                            <div class="alert alert-danger pt-1 pb-1">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="row">
                          @foreach($permissionParentItem->permissionChildren as $permissionChildrenItem) 
                            <div class="card-body text-primary col-md-3">
                              <h5 class="card-title">
                                <input class="checkbox_children" type="checkbox" name="permission_id[]" value="{{ $permissionChildrenItem->id }}"/>
